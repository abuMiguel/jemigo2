...
export class Api {
  ...
  setupBlogDataRoute(db: GNookDb) {
    ...
    this.router.get("/blog/data/:id", async function (req: Request, res: Response) {
        try {
          ...
          const blogData = await db.getBlogDataByPath(id);
  ...
  setupAmazonApiRoutes(db: GNookDb) {
